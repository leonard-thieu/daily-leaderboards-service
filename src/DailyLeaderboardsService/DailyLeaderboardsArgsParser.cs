...
{
    internal sealed class DailyLeaderboardsArgsParser : ArgsParser<DailyLeaderboardsOptions, IDailyLeaderboardsSettings>
    {
        public DailyLeaderboardsArgsParser(TextReader inReader, TextWriter outWriter, TextWriter errorWriter) : base(inReader, outWriter, errorWriter) { }

        protected override string EntryAssemblyFileName { get; } = Path.GetFileName(Assembly.GetExecutingAssembly().Location);

        protected override void OnParsing(Type settingsType, OptionSet optionSet, DailyLeaderboardsOptions options)
        {
        ...
        protected override void OnParsed(DailyLeaderboardsOptions options, IDailyLeaderboardsSettings settings)
        {
